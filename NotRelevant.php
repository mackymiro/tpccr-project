...
?>
        <div class="col-md-12">
                    ...
                    <select name="QCStatus" id="QCStatus" >
                      <?php
                        $QCStatus= $_POST['QCStatus'];
                          ...
                          if ($QCStatus!=''){
                            echo "<option value='".$QCStatus."' selected>".$QCStatus."</option>";
                          }
                      ?> 
                    </select>
          ...
          if ($_POST['Search']==''){
              $strSQL = "SELECT * From  primo_view_NotRelevant where isnull(QCStatus,'')='".$QCStatus."'";
          }
          else{
            $strSQL="SELECT * From  primo_view_NotRelevant Where  Jurisdiction='".$_POST['Search']."' AND isnull(QCStatus,'')='".$QCStatus."'";   
          }
...
?>
                <tr id="<?php echo $row["BatchId"];?>">
                  <td><a href="index.php?file=<?=$filename;?>&BatchID=<?php echo $row["BatchId"];?>&Task=CONTENTREVIEW" target="_blank"><?php echo $row["JobName"];?></td>
                  <td><?php echo $row["Jurisdiction"];?></td>
                  <td><a href="<?php echo $row["SourceUrl"];?>" target="_blank"><?php echo $row["Title"];?></a></td>
                  ...
                  <td><?php echo $row["AssignedTo"];?></td>
                   ...
                   if ($QCStatus==''){
                    ...
                    ?>
                   <td><button class="btn-info" onclick='SetTextBoxBatch("<?php echo $row["BatchId"];?>","<?php echo $row["JobId"];?>")' type="button" data-toggle="modal" data-target="#modal-Verify" >Verify</button></td>
  ...
  <div class="modal modal-primary fade" id="modal-Verify">
        <div class="modal-dialog">
        <div class="modal-content">
          <div class="modal-header">
          <button type="button" class="close" data-dismiss="modal" aria-label="Close">
            <span aria-hidden="true">&times;</span></button>
          <h4 class="modal-title">Verify Relevancy</h4>
          </div>
          <div class="modal-body">
            <input type="hidden" class="form-control" name="BatchID" id="BatchID" value="<?php echo "$BatchID";?>">
             <input type="hidden" class="form-control" name="BatchID" id="JobID"  >
          <p>Please verify relevancy. This was tag as Not Relevant.</p>
          </div>
          <div class="modal-footer">
          <button type="button" class="btn btn-outline pull-left" data-dismiss="modal" onclick="Verified()">Verify</button>
          <button type="button" onclick="MoveToRelevant()" class="btn btn-outline" data-dismiss="modal" >Relevant</button>
          </div>
        </div>
        <!-- /.modal-content -->
        </div>
...
<script type="text/javascript">  
  function Verified(){
        ...
        var data = 'data='+BatchID+"&JobID="+JobID;
      ...
      function MoveToRelevant(){
       ...
       function SetTextBoxBatch(prVal,prVal1) {
       
            document.getElementById('BatchID').value = prVal;
            document.getElementById('JobID').value = prVal1;
