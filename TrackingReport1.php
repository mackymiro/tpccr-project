    ...
    <section class="content">
      <div class="row">
        ...
        <?php
            ...
            $strSQL="SELECT  * from tblState Where Status='True' ORDER BY StateName ASC";
        ...
        $strSQL="SELECT  * from primo_view_jobs where ProcessCode='$search' AND DateRegistered>='$prDateFrom' AND DateRegistered<='$prDateTO' AND State='$StateName'";
      }
				$objExec= odbc_exec($conWMS,$strSQL);
...
?>
                <tr>
                  ...
                  <td><?php echo $row["HoldRemarks"];?></td>
                  <td><?php echo $row["DateRegistered"];?></td>
                  <td><?php 
                  if ($status=='New'){
                  ?>
                  <button type="button" id="btnAllocate<?= $row["BatchId"]?>" class="btn btn-success small"  data-toggle="modal" data-target="#modal-success"  onclick="Javascript:SetTextBoxValue(<?php echo $BatchID;?>)">Allocate</button>
                  <?php
                  }
                  elseif($status=='Hold'){
                  ?>
