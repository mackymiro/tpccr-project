          ...
          <div class="box box-primary">
              ...
              <form method="post" action="">
                ...
                <label> Priority:</label>
                    <select name="Search" id="Search" >
                      <?php
                      $sql="Select DISTINCT TransPriority From primo_view_Jobs ORDER BY TransPriority";
                    $rs=odbc_exec($conWMS,$sql);
                        $ctr = odbc_num_rows($rs);
                        while(odbc_fetch_row($rs))
                        {
                          $Val=odbc_result($rs,"TransPriority");
			...
			<form method="post" action="Deliver.php">
              ...
              <table id="example1" class="table table-bordered table-striped">
				...
				//$strSQL="SELECT * FROM primo_view_Jobs Where ProcessCode='ENRICHMENT'";
				// if ($_SESSION['UserType']=='Admin'){
					// $strSQL="SELECT * FROM primo_view_Record Where JobStatus='NEW'";	

        if ($_SESSION['UserType']=='Admin'){

          if ($_POST['Search']==''){
            $strSQL="SELECT  * FROM primo_view_jobs Where ProcessCode='STYLING' AND statusstring='DONE' and status<>'Transmitted'";  
          }
          else{
            $strSQL="SELECT * FROM primo_view_jobs Where ProcessCode='STYLING' AND statusstring='DONE' and  status<>'Transmitted' AND Jurisdiction='".$_POST['Search']."'";   
          }
         
        }
        else{
          if ($_POST['Search']==''){
            $strSQL="SELECT * FROM primo_view_jobs Where ProcessCode='STYLING' AND statusstring='DONE' and status<>'Transmitted' AND AssignedTo='$_SESSION[login_user]'";  
          }
          else{

             $strSQL="SELECT  * FROM primo_view_jobs Where ProcessCode='STYLING' AND statusstring='DONE' and status<>'Transmitted' AND AssignedTo='$_SESSION[login_user]' AND Jurisdiction='".$_POST['Search']."'";  
          }
